  ...
  if (count === 0) {
    return (
      <main className="mx-auto flex w-full max-w-lg flex-col items-center px-5 py-20 text-center md:py-28">
        <Icon name="heart" size={34} className="text-ink-faint" />
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-5 py-8 md:px-8 md:py-12">
        ...
        {items.map((product) => (
          ...
          >
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-between">
              <div>
                <p className="meta text-[0.5625rem]">
                  {titleCase(product.category)}
                </p>
                <Link
                  href={`/product/${product.slug}`}
                  className="mt-0.5 block font-serif text-base leading-snug text-ink text-pretty"
                >
                  {product.name}
                </Link>
                <p className="mt-0.5 font-sans text-[0.6875rem] font-light text-ink-muted">
                  {product.metal}
                </p>
              </div>
            ...
            <button
              ...
              className="-mr-2 -mt-2 size-11 shrink-0 self-start text-ink-faint transition-colors duration-150 hover:text-ink"
            >
      ...
      <div className="mt-6 flex items-baseline justify-between">
        <p className="meta">Indicative total</p>
        <p className="font-sans text-lg tabular-nums text-ink">
          {formatPrice(total)}
        </p>
      </div>
      <p className="mt-1 text-right font-sans text-[0.6875rem] font-light text-ink-muted">
      ...
      <div className="sticky bottom-16 z-raised mt-8 -mx-5 border-t border-hairline bg-canvas/95 px-5 py-4 backdrop-blur-md md:static md:mx-0 md:border-0 md:bg-transparent md:p-0 md:backdrop-blur-none">
        <a
        ...
        <div className="mt-3 flex items-center justify-between">
          <Link
            href="/collections"
            className="font-sans text-[0.6875rem] uppercase tracking-[0.16em] text-ink-muted transition-colors duration-150 hover:text-ink"
          >
            Keep browsing
          </Link>
          <button
            ...
            className="font-sans text-[0.6875rem] uppercase tracking-[0.16em] text-ink-faint transition-colors duration-150 hover:text-danger"
          >
            Clear all
          </button>
        </div>
